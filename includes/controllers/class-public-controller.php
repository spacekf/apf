...
namespace SW_WAPF_PRO\Includes\Controllers {
    ...
    class Public_Controller {
        ...
        public function ajax_upload_remove() {
            ...
            $verify_nonce = apply_filters( 'wapf/file/check_nonce', false ); 

            	        if( $verify_nonce && ( ! isset( $_GET['nonce'] ) || ! wp_verify_nonce($_GET['nonce'], 'wapf_fupload') || ! isset( $_GET['file'] ) ) ) {
		        wp_send_json_error();
	        }
        ...
        public function ajax_upload() {
	        ...
	        if( $verify_nonce && ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'wapf_fupload' ) || empty( $_POST['field_groups'] ) ) ) {
		        wp_send_json_error();
	        }
	        ...
	        $res = [];
	        	...
	        	foreach( $files_arr as $f ) {
                    $res[] = apply_filters( 'wapf/file/ajax_upload_success_file_result', [
        ...
        public function register_assets() {
            ...
            $path       = wapf_get_setting( 'path' ) . 'assets/';
            $version    = wapf_get_setting( 'version' );

                        if( ! file_exists( $path . 'css/frontend.min.css' ) ) {
                Design_Helper::generate_css();
            }

            wp_enqueue_style( 'wapf-frontend', $url . 'css/frontend.min.css', [], $version . '-' . filemtime( $path . 'css/frontend.min.css' ) );
            ...
            $script_vars = [
                ...
                'today'             => $today_server->format('Y-m-d\T00:00:00'),
