...
namespace SW_WAPF_PRO\Includes\Classes {
	...
	class File_Upload {
		...
		public static function get_htaccess_rules() {
			...
			$filetypes = '';
			$restricted_filetypes = self::$field_for_upload ? self::get_allowed_filetypes( self::$field_for_upload ) : [];
		...
		public static function get_base_upload_dir(): string {
			$wp_upload_dir = wp_upload_dir();
			return trailingslashit( $wp_upload_dir['basedir'] ) . trailingslashit(self::$upload_parent_dir);
		}

        public static function get_base_upload_url(): string {

                        static $cached_url = null;

            if ( $cached_url === null ) {
                $wp_upload_dir = wp_upload_dir();
                $cached_url = trailingslashit( $wp_upload_dir['baseurl'] ) . trailingslashit( self::$upload_parent_dir );
            }
		...
		public static function handle_upload($file, Field $field): array {
			...
			if( apply_filters( 'wapf/file/test_file_type', true ) && empty( $file_info['type'] ) ) {
            ...
            if( ! empty( $upload['url'] ) && strpos( $upload['url'], 'http://' ) !== false && is_ssl() ) {
		...
		public static function get_allowed_filetypes(Field $field): array {
		...
		public static function get_all_allowed_filetypes(): array {
		...
		public static function validate_files_for_field($files,Field $field,$clone_idx = 0) {
			...
			if(empty($field->options['multiple']) && $total_files> 1) {
				return apply_filters('wapf/message/upload_err_too_many', __("You are not allowed to upload multiple files.",'sw-wapf'));
			}
			...
			if( $total_files_without_error > intval(ini_get('max_file_uploads'))) {
				return apply_filters('wapf/message/upload_err_uploads_exceeded', __("The maximum number of allowed simultanious uploads was exceeded. Please upload less files.",'sw-wapf'));
			}
		...
		public static function is_ajax_upload() {
			...
			return apply_filters( 'wapf_upload_ajax', $enable_ajax === 'yes' );
		}
