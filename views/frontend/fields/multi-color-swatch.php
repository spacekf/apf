...
if(!empty($model['field']->options['choices'])) {
    ...
    foreach ($model['field']->options['choices'] as $option) {
        ...
        echo sprintf(
            '<div class="wapf-swatch wapf-swatch--color %s" data-dir="t"><label aria-label="%s"><div style="%sbackground-color: %s;width:%spx;height:%spx" class="wapf-color wapf--%s"></div><input type="checkbox" autocomplete="off" %s />%s</label></div>',
