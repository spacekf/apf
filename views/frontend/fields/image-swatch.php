...
$classes        = 'wapf-swatch wapf-swatch--image wapf-single-select' . ( $label_outside ? '' : ' apf-pick-box' );
...
if( ! empty( $model['field']->options['choices'] ) ) {
	...
	foreach ($model['field']->options['choices'] as $option) {
		...
		echo sprintf(
			'<div class="%s %s" %s><label aria-label="%s"><input type="radio" %s /><div %s>%s</div>%s</label></div>',
			$classes,
